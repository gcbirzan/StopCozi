...
var tz, firstDay;
  ...
  return {
    getVisibleMinutes: function (m, step) {
      m = m && m.isValid() ? m : (tz ? moment.tz(tz).day(firstDay) : moment().day(firstDay));
      var year = m.year(),
        month = m.month(),
        day = m.date(),
  ...
  return {
    ...
    link: function (scope, element, attrs, ngModel) {
      ...
      function formatter(value) {
        var ret = dateFilter(value, format, timezone);

        if (ret == moment.localeData().invalidDate()) {
          ret = null;
        }
      ...
      function parser(viewValue) {
        if ((viewValue = moment(viewValue, format)).isValid()) {
          return viewValue;
        }
        return null;
      }
